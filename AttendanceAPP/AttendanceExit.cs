...
namespace AttendanceAPP
{
    public partial class AttendanceExit : UserControl
    {
                    ...
                    using (SolidBrush brush = new SolidBrush(Color.Yellow))
                    {
                        ...
                        if (!string.IsNullOrEmpty(matchedUser))
                        {
                            if (result.Status == AntiSpoofingStatus.Real && !string.IsNullOrEmpty(matchedUser))
                            {
                                ...
                                Cframe = Attendance.CropImage(frame, Rect);
                                byte[] imageBytes = Attendance.ImageToByteArray(Cframe);
                                Attendance.MarkAttendance(matchedUser, imageBytes);
                                Attendance.UpdateExitTime(matchedUser);
                                matchedUsersCount++;
        ...
        private Dictionary<string, List<float[]>> LoadFaceDataFromDatabase()
        {
            ...
            {
                conn.Open();
                string query = "SELECT Users.UserName, UserFaceFeatures.MaskFeatures FROM Users INNER JOIN UserFaceFeatures ON Users.UserId = UserFaceFeatures.UserId";
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        byte[] faceBytes = reader["MaskFeatures"] as byte[];

                        if (faceBytes != null && faceBytes.Length % sizeof(float) == 0)
                        {
                            float[] faceFeature = Attendance.ConvertBytesToFloatArray(faceBytes);
                            if (faceFeature != null)
                            {
