...
namespace AttendanceAPP
{
    public partial class UserProfile : UserControl
    {
        ...
        private FaceRecognizer faceRecognizer, maskRecognizer;
        ...
        private void UserProfile_Load(object sender, EventArgs e)
        {
           videoDevices = new FilterInfoCollection(FilterCategory.VideoInputDevice);
            ...
            maskRecognizer = new FaceRecognizer(new FaceRecognizeConfig(FaceType.Mask));
            videoSource = new VideoCaptureDevice(videoDevices[0].MonikerString);
            videoSource.NewFrame += videoSource_NewFrame;
            dataGrid.CellClick += dataGrid_CellContentClick;

            Loader.SetDefaultCameraResolution(videoSource, 640, 480);
            Loader.LoadUserData(dataGrid);
        ...
        {
            Cframe = Attendance.CropImage((Bitmap)pictureBox.Image, faceRectangle);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            float[] maskFeature = ExtractSinglemaskFeature(Cframe);
            SaveUserToDatabase(userid, username, userGender, userAge, faceFeature, maskFeature);
            Loader.LoadUserData(dataGrid);
        ...
        private void SaveUserToDatabase(int userId, string username, string gender, int age, float[] faceFeature, float[] maskFeature)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string featureQuery = "INSERT INTO UserFaceFeatures (UserId, FeatureData,MaskFeatures) VALUES (@UserId, @FeatureData,@MaskFeatures)";
                    using (SqlCommand cmd = new SqlCommand(featureQuery, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@MaskFeatures", FloatArrayToByteArray(maskFeature));
                        cmd.ExecuteNonQuery();
                    }
        ...
        private float[] ExtractSinglemaskFeature(Bitmap image)
        {
            if (maskRecognizer == null || maskRecognizer == null) return null;
            ...
            return maskRecognizer.Extract(imageData, landmarks);
        }
